...
extern const char *const kKotlinCoders;
extern const char *const kKotlinEnum;
extern const char *const kKotlinStruct;
extern const char *const kKotlinServer;
extern const char *const kKotlinClient;
extern const char *const kKotlinConst;
extern const char *const kKotlinInterface;

////////////////////////////////////////////////////////////////////////////////
// Code
////////////////////////////////////////////////////////////////////////////////

KotlinGenerator::KotlinGenerator(InterfaceDefinition *def, const std::string kotlinPackageName) :
Generator(def, generator_type_t::kKotlin), m_kotlinPackageName(kotlinPackageName), m_suffixStrip(""), m_suffixStripSize(0)
{
    /* Set copyright rules. */
...
void KotlinGenerator::generateOutputFiles(const string &fileName)
{
...
void KotlinGenerator::generateCommonFile(string fileName)
{
...
void KotlinGenerator::generateConstFile(string fileName)
{
    fileName += "/common/";
    ...
    std::string classFilename = fileName + "/Constants.kt";
    generateOutputFile(classFilename, "kotlin_const", m_templateData, kKotlinConst);
}

void KotlinGenerator::generateClientFile(string fileName)
{
    ...
    for (data_ptr interface : list)
    {
        ...
        std::string classFilename = fileName + "/" + name + "Client.kt";
        generateOutputFile(classFilename, "kotlin_client", m_templateData, kKotlinClient);
    }
}

void KotlinGenerator::generateServerFile(string fileName)
{
    data_map &map = m_templateData["group"].get().get()->getmap();
    data_list &list = map["interfaces"]->getlist();
    fileName += "/server";
    ...
    for (data_ptr interface : list)
    {
        std::string name = interface.get().get()->getmap()["name"].get().get()->getvalue();
        m_templateData["interface"] = interface;
        std::string classFilename = fileName + "/Abstract" + name + "Service.kt";
        generateOutputFile(classFilename, "kotlin_server", m_templateData, kKotlinServer);
    }
}

void KotlinGenerator::generateInterfaceFile(string fileName)
{
    data_map &map = m_templateData["group"].get().get()->getmap();
    data_list &list = map["interfaces"]->getlist();
    fileName += "/interfaces";
    ...
    for (data_ptr interface : list)
    {
        std::string name = interface.get().get()->getmap()["name"].get().get()->getvalue();
        m_templateData["interface"] = interface;
        std::string classFilename = fileName + "/I" + name + ".kt";
        generateOutputFile(classFilename, "kotlin_interface", m_templateData, kKotlinInterface);
    }
}

void erpcgen::KotlinGenerator::generateEnumFiles(std::string fileName)
{
    ...
    for (data_ptr item : list)
    {
        ...
        std::string classFilename = fileName + "/" + name + ".kt";
        generateOutputFile(classFilename, "kotlin_enum", m_templateData, kKotlinEnum);
    }
}

void erpcgen::KotlinGenerator::generateStructFiles(std::string fileName)
{
    ...
    for (data_ptr item : list)
    {
        ...
        // dump_data(item);
        std::string classFilename = fileName + "/" + name + ".kt";
        generateOutputFile(classFilename, "kotlin_struct", m_templateData, kKotlinStruct);
    }
}

void KotlinGenerator::parseSubtemplates()
{
    const char *templateName = "kotlin_coders";
    try
    {
        parse(kKotlinCoders, m_templateData);
    }
...
void KotlinGenerator::generate()
{
    ...
    m_templateData["appPackage"] = make_data(m_kotlinPackageName);
...
void KotlinGenerator::setTemplateComments(Symbol *symbol, data_map &symbolInfo)
{
    symbolInfo["mlComment"] = symbol->getMlComment();
    symbolInfo["ilComment"] = symbol->getIlComment();
}

data_map KotlinGenerator::getFunctionTemplateData(Group *group, Function *fn)
{
    (void)group;
    data_map info;
    ...
    for (StructMember *param : fnParams)
    {
        ...
        bool isNullable = ((findAnnotation(param, NULLABLE_ANNOTATION) != nullptr) &&
                           (trueDataType->isStruct() || trueDataType->isString() ||
                           (!trueDataType->isBuiltin() && !trueDataType->isEnum())));
...
string KotlinGenerator::getFunctionPrototype(Group *group, FunctionBase *fn, const string &interfaceName,
                                           const string &name, bool insideInterfaceCall)
{
    ...
    proto = "suspend fun " + getOutputName(function);
    ...
    string typeName = getTypenameName(dataTypeReturn, false, false);
    if (typeName == "void") {
        proto += ")";
    } else {
        proto += "): ";
        proto += typeName + (isNullable ? "?" : "");
    }
    return proto;
}

string KotlinGenerator::getTypenameName(DataType *t, bool isReference, bool objectType)
{
    string returnName;
...
string KotlinGenerator::getArrayInitialization(ArrayType *t)
{
    string result = "";
    ...
    {
        result = "List" + result; // Kotlin does not support typed arrays e.g. List<int[]>. Use untyped List[].
    }
    else
    {
...
void KotlinGenerator::makeConstTemplateData()
{
...
void KotlinGenerator::makeEnumsTemplateData()
{
    Log::info("Enums:\n");
    data_list enums;
    int n = 0;
...
data_list KotlinGenerator::getEnumMembersTemplateData(EnumType *enumType)
{
    int j = 0;
...
void KotlinGenerator::makeAliasesTemplateData()
{
...
data_map KotlinGenerator::makeGroupSymbolsTemplateData(Group *group)
{
...
void KotlinGenerator::setStructMembersTemplateData(StructType *structType, data_map &structInfo)
{
    data_list members;
...
void KotlinGenerator::setOneStructMemberTemplateData(StructMember *member, data_map &member_info)
{
    ...
    bool isNullable = ((findAnnotation(member, NULLABLE_ANNOTATION) != nullptr) &&
                       (trueDataType->isStruct() || trueDataType->isBinary() ||
                       trueDataType->isString() || trueDataType->isList()));
...
data_map KotlinGenerator::getEncodeDecodeCall(const string &name, DataType *t, StructType *structType,
                                            ...
                                            bool needTypeDeclaration, bool isFunctionParam)
{
    ...
    templateData["isNeedProperty"] = true;
    ...
    switch (t->getDataType())
    {
        ...
        {
            throw internal_error("Kotlin does not support functions yet.");
        }
        case DataType::data_type_t::kListType:
        {
...
string KotlinGenerator::capitalize(string text)
{
    text[0] = toupper(text[0]);
    return text;
}

void KotlinGenerator::makeFunctionsTemplateData()
{
...
data_map KotlinGenerator::getTypeInfo(DataType *t, int isReference, bool inDataContainer)
{
    data_map info;
    std::string name = filterName(getOutputName(t, false));
    info["name"] = name;
    info["typeName"] = getTypenameName(t, isReference, isReference); // Kotlin type: Float, List<int[]>, ...
...
string KotlinGenerator::getBuiltinCodecTypeName(const BuiltinType *t)
{
    switch (t->getBuiltinType())
    {
        ...
        case BuiltinType::builtin_type_t::kUInt64Type:
            throw internal_error("Kotlin implementation does not support uint64");
...
string KotlinGenerator::getBuiltinTypename(const BuiltinType *t)
{
    switch (t->getBuiltinType())
    {
        case BuiltinType::builtin_type_t::kBoolType:
...
string KotlinGenerator::getBuiltinObjectTypename(const BuiltinType *t)
{
    switch (t->getBuiltinType())
    {
        case BuiltinType::builtin_type_t::kBoolType:
...
string KotlinGenerator::filterName(const string &name)
{
    string result = name;
...
std::string erpcgen::KotlinGenerator::getGroupPackageName(const Group *group)
{
    string name = m_kotlinPackageName;
...
bool KotlinGenerator::checkWhitespaceChar(char c)
{
    if (c == ' ' || c == '\t' || c == '\n' || c == '\r')
    {
        return true;
...
string KotlinGenerator::stripWhitespace(const string &s)
{
    string result = s;
    int32_t i;
    uint32_t n;
...
void KotlinGenerator::initKotlinReservedWords()
{
