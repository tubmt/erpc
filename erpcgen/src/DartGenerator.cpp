...
extern const char *const kDartCoders;
extern const char *const kDartEnum;
extern const char *const kDartStruct;
extern const char *const kDartServer;
...
DartGenerator::DartGenerator(InterfaceDefinition *def, const std::string dartPackageName) :
Generator(def, generator_type_t::kDart), m_dartPackageName(dartPackageName), m_suffixStrip(""), m_suffixStripSize(0)
{
    /* Set copyright rules. */
...
void DartGenerator::generateOutputFiles(const string &fileName)
{
...
void DartGenerator::generateConstFile(string fileName)
{
    fileName += "/common/";
...
void DartGenerator::generateClientFile(string fileName)
{
...
void DartGenerator::generateServerFile(string fileName)
{
    data_map &map = m_templateData["group"].get().get()->getmap();
    data_list &list = map["interfaces"]->getlist();
    fileName += "/server";
...
void DartGenerator::generateInterfaceFile(string fileName)
{
    data_map &map = m_templateData["group"].get().get()->getmap();
    data_list &list = map["interfaces"]->getlist();
    fileName += "/interfaces";
...
void erpcgen::DartGenerator::generateEnumFiles(std::string fileName)
{
...
void erpcgen::DartGenerator::generateStructFiles(std::string fileName)
{
...
{
    const char *templateName = "dart_coders";
    try
    {
        parse(kDartCoders, m_templateData);
    }
...
void DartGenerator::generate()
{
    ...
    m_templateData["appPackage"] = make_data(m_dartPackageName);
...
void DartGenerator::setTemplateComments(Symbol *symbol, data_map &symbolInfo)
{
    symbolInfo["mlComment"] = symbol->getMlComment();
    symbolInfo["ilComment"] = symbol->getIlComment();
}

data_map DartGenerator::getFunctionTemplateData(Group *group, Function *fn)
{
    (void)group;
    data_map info;
...
string DartGenerator::getFunctionPrototype(Group *group, FunctionBase *fn, const string &interfaceName,
...
string DartGenerator::getTypenameName(DataType *t, bool isReference, bool objectType)
{
    string returnName;
...
string DartGenerator::getArrayInitialization(ArrayType *t)
{
    string result = "";
    ...
    {
        result = "List" + result; // Dart does not support typed arrays e.g. List<int[]>. Use untyped List[].
    }
    else
    {
...
void DartGenerator::makeEnumsTemplateData()
{
    Log::info("Enums:\n");
    data_list enums;
    int n = 0;
...
data_list DartGenerator::getEnumMembersTemplateData(EnumType *enumType)
{
    int j = 0;
...
void DartGenerator::makeAliasesTemplateData()
{
...
data_map DartGenerator::makeGroupSymbolsTemplateData(Group *group)
{
...
void DartGenerator::setStructMembersTemplateData(StructType *structType, data_map &structInfo)
{
    data_list members;
...
void DartGenerator::setOneStructMemberTemplateData(StructMember *member, data_map &member_info)
{
...
data_map DartGenerator::getEncodeDecodeCall(const string &name, DataType *t, StructType *structType,
                                            ...
                                            bool needTypeDeclaration, bool isFunctionParam)
{
    ...
    switch (t->getDataType())
    {
        ...
        {
            throw internal_error("Dart does not support functions yet.");
        }
        case DataType::data_type_t::kListType:
        {
...
string DartGenerator::capitalize(string text)
{
    text[0] = toupper(text[0]);
    return text;
}

void DartGenerator::makeFunctionsTemplateData()
{
...
data_map DartGenerator::getTypeInfo(DataType *t, int isReference, bool inDataContainer)
{
    data_map info;
    std::string name = filterName(getOutputName(t, false));
    info["name"] = name;
    info["typeName"] = getTypenameName(t, isReference, isReference); // Dart type: Float, List<int[]>, ...
...
string DartGenerator::getBuiltinCodecTypeName(const BuiltinType *t)
{
...
string DartGenerator::getBuiltinTypename(const BuiltinType *t)
{
    switch (t->getBuiltinType())
    {
        case BuiltinType::builtin_type_t::kBoolType:
...
string DartGenerator::getBuiltinObjectTypename(const BuiltinType *t)
{
    switch (t->getBuiltinType())
    {
        case BuiltinType::builtin_type_t::kBoolType:
...
string DartGenerator::filterName(const string &name)
{
    string result = name;
...
std::string erpcgen::DartGenerator::getGroupPackageName(const Group *group)
{
    string name = m_dartPackageName;
    string outputDir = getAnnStringValue(m_def->getProgramSymbol(), OUTPUT_DIR_ANNOTATION);
    ...
    else
    {
        ...
        {
            string fileName = m_def->getOutputFilename();
            size_t pos = fileName.find(".erpc");
...
bool DartGenerator::checkWhitespaceChar(char c)
{
    if (c == ' ' || c == '\t' || c == '\n' || c == '\r')
    {
        return true;
...
string DartGenerator::stripWhitespace(const string &s)
{
    string result = s;
    int32_t i;
    uint32_t n;
...
void DartGenerator::initDartReservedWords()
{
